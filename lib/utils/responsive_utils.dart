...
class ResponsiveUtils {
  ...
  static double width(BuildContext context, double size) {
    ...
    if (isMobile(context))
      return size * (screenWidth / 375); // Base mobile width
    if (isTablet(context))
      return size * (screenWidth / 768); // Base tablet width
    return size * (screenWidth / 1440); // Base desktop width
  }

  static double height(BuildContext context, double size) {
    final screenHeight = MediaQuery.of(context).size.height;
    if (isMobile(context))
      return size * (screenHeight / 812); // Base mobile height
    if (isTablet(context))
      return size * (screenHeight / 1024); // Base tablet height
    return size * (screenHeight / 900); // Base desktop height
  }
